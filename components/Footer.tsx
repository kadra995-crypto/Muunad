"use client";

export default function Footer() {
  return (
    <footer id="about" className="bg-natural text-white">
      {/* About section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-16">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {/* Brand */}
          <div className="lg:col-span-2">
            <div className="mb-4">
              <span className="font-display text-3xl font-bold tracking-widest text-white">MUUNAD</span>
              <p className="text-[10px] tracking-[0.3em] text-quality font-medium uppercase mt-0.5">
                Pure Beauty
              </p>
            </div>
            <p className="text-white/60 text-sm leading-relaxed max-w-sm">
              MUUNAD brings premium K-beauty and international skincare to Somalia.
              We curate the best products for our climate — SPF-focused, skin-type specific,
              and always natural-first.
            </p>
            <div className="flex flex-wrap gap-2 mt-4">
              {["Natural", "Minimal", "Elegant", "Warm", "Premium"].map((vibe) => (
                <span key={vibe} className="text-[10px] px-3 py-1 border border-white/20 rounded-full text-white/60">
                  {vibe}
                </span>
              ))}
            </div>
            {/* WhatsApp */}
            <a
              href="https://wa.me/252615000000?text=Hello%20MUUNAD!"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 mt-5 bg-green-500 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-green-600 transition-colors"
            >
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
              </svg>
              +252 61 500 0000
            </a>
          </div>

          {/* Quick Links */}
          <div>
            <h4 className="font-semibold text-white mb-4 text-sm tracking-wide uppercase">Shop</h4>
            <ul className="space-y-2.5 text-sm text-white/60">
              {["All Products", "SPF & Sunscreen", "Serums & Essences", "Cleansers", "Moisturizers", "Treatments"].map((link) => (
                <li key={link}>
                  <button
                    onClick={() => document.getElementById("products")?.scrollIntoView({ behavior: "smooth" })}
                    className="hover:text-quality transition-colors"
                  >
                    {link}
                  </button>
                </li>
              ))}
            </ul>
          </div>

          {/* Payment & Contact */}
          <div>
            <h4 className="font-semibold text-white mb-4 text-sm tracking-wide uppercase">Payment Methods</h4>
            <div className="space-y-3">
              {[
                { emoji: "📱", label: "EVC Plus", desc: "Hormuud Telesom" },
                { emoji: "🏦", label: "Sahal", desc: "Premier Bank" },
                { emoji: "📲", label: "Zaad", desc: "Telesom" },
                { emoji: "💳", label: "Visa / Mastercard", desc: "Credit & Debit" },
              ].map((pm) => (
                <div key={pm.label} className="flex items-center gap-2.5">
                  <span className="text-xl">{pm.emoji}</span>
                  <div>
                    <p className="text-xs font-semibold text-white">{pm.label}</p>
                    <p className="text-[10px] text-white/40">{pm.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>

      {/* SPF Note */}
      <div className="border-t border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-white/40">
            <span>☀️</span>
            <span>Somalia UV Index 10–12 year-round — SPF is a daily necessity, not a luxury.</span>
          </div>
          <p className="text-xs text-white/30">
            © {new Date().getFullYear()} MUUNAD. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
  );
}
